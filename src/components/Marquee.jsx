...
const Marquee = () => (
  <div className="relative w-full overflow-hidden border-y border-[color:var(--color-border)] bg-white/[0.02] py-4">
    <motion.div
      className="flex gap-6 whitespace-nowrap font-display text-sm md:text-base font-medium text-[var(--color-muted)]"
      animate={{ x: ['0%', '-50%'] }}
      transition={{ duration: 22, repeat: Infinity, ease: 'linear' }}
    >
      {[...items, ...items].map((item, i) => (
        <span key={i} className={item === '✦' ? 'text-[var(--color-accent)]' : ''}>{item}</span>
      ))}
    </motion.div>
  </div>
);
